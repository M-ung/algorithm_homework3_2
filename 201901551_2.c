...
int palindrome(char *s, int i, int j); // 가장 긴 회문의 길이를 구하는 함수
void printDpTable(int n, int dp[n][n]); // dp 테이블 출력 함수의 원형

char str[] = "KBCDXBMBXCABAD"; // 입력할 문자열
...
int palindrome(char *s, int i, int j) {
    ...
    printDpTable(n, dp); // dp 테이블 출력 함수 호출
...
// dp 테이블을 출력하는 함수
void printDpTable(int n, int dp[n][n]) {
    printf("DP Table:\n");
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            printf("%d ", dp[i][j]);
